...
\begin{frame}{Organização Geral}
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[
            ...
            \node {tj\_scraper}
                child {node [extract] {.cache}}
                child {node [interface] {.cli}}
                child {node [extract] {.download}}
                child {node [utils] {.errors}}
                child {node [extract] {.export}}
                child {node [extract] {.html}}
                child {node {.process}}
                child {node [utils] {.statistics}}
                child {node [utils] {.timing}}
                child {node [utils] {.url}}
                child {node [blue, fill=blue!20] {.webapp}}
                ;
        \end{tikzpicture}
    \end{figure}
\end{frame}
